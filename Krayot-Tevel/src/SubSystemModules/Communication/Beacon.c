...
{
	if(FRAM_read(&g_beacon_change_baud_period,BEACON_BITRATE_CYCLE_ADDR,BEACON_BITRATE_CYCLE_SIZE) != 0)
	{
		g_beacon_change_baud_period = DEFALUT_BEACON_BITRATE_CYCLE;
	}

	if (FRAM_read((unsigned char*) &g_beacon_interval_time,BEACON_INTERVAL_TIME_ADDR,BEACON_INTERVAL_TIME_SIZE) != 0)
	{
...
int BeaconSetBitrate()
{
	...
	int err = IsisTrxvu_tcSetAx25Bitrate(ISIS_TRXVU_I2C_BUS_INDEX, trxvu_bitrate_9600);
	//}
...
void BeaconLogic()
{
	...
	sat_packet_t cmd = { 0 };
	int err = AssembleCommand((unsigned char*) &wod, sizeof(wod), trxvu_cmd_type, BEACON_SUBTYPE, 0xFFFFFFFF, &cmd);
	if (0 != err) {
		return;
	}
...
int UpdateBeaconBaudCycle(unsigned char cycle)
{
	...
	int err = FRAM_write((unsigned char*) &cycle, BEACON_BITRATE_CYCLE_ADDR, BEACON_BITRATE_CYCLE_SIZE);
